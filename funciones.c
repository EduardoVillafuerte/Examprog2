...
void buscarXPrecioMenor(char productos[][3][40],double precio[]){
    int preciomenor = 100000, indice = 0;
    printf("El o los productos con menor precio encontrados son:\n");
    printf("Nombre\t     Categoria\t Marca \t Precio\n");

    for(int k = 0; k <10; k++){
        if(precio[k] < preciomenor){
            preciomenor = precio[k];
            indice = k;
        }
        else if(preciomenor == precio[k]){
            printf("%s     %s     %s     %.2f\t\n",productos[k][0],productos[k][1],productos[k][2],precio[k]);
        }
    }
    printf("%s     %s     %s     %.2f\t\n",productos[indice][0],productos[indice][1],productos[indice][2],precio[indice]);
}
